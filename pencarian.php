...
<ul class="list-group">
  ...
  if(isset($_GET['cari'])){
    $cari = $_GET['cari'];
    $d = mysqli_query($connect, " SELECT * from karya_ilmiah  INNER JOIN tipe ON id_tipe= idtipe
                              INNER JOIN tahun ON id_tahun= tahun where judul LIKE '%$cari%' OR nama_penulis LIKE '%$cari%' OR th_tahun LIKE '%$cari%' OR lembaga_penulis LIKE '%$cari%'  OR nama_tipe LIKE '%$cari%'  ");       
  }else{
    $d = mysqli_query($connect, " SELECT * from karya_ilmiah  INNER JOIN tipe ON id_tipe= idtipe
